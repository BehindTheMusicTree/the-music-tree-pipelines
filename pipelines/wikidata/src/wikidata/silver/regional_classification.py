...
def classify_regional_genres(genre_parents_path: Path, output_dir: Path) -> Path:
    logger.info("classifying regional genres in %s", genre_parents_path)
    df = pl.read_parquet(genre_parents_path)

    # Seeds: the "music of <place>" items themselves (excluded, not genres). A genre item is
    # "direct" regional if any one of its parent edges points at a seed — not all of them, since
    # e.g. "European folk music" has one parent into "music of Europe" (regional) and another into
    ...
    seed_ids = set(df.filter(pl.col("exclusion_reason") == "regional_overview").select("item_id").unique().to_series())
    direct_ids = set(
    ...
    df = df.with_columns(
        is_regional=pl.when(~pl.col("is_genre")).then(None).otherwise(pl.col("item_id").is_in(list(regional_ids))),
        regional_reason=pl.when(pl.col("item_id").is_in(list(direct_ids)))
        .then(pl.lit("direct"))
    ...
    logger.info(
        "wrote %d rows to %s (%d regional genre items: %d direct, %d inherited)",
        df.height,
        output_path,
        len(regional_ids),
        len(direct_ids),
