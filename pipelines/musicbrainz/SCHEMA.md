...
`3_songs` (`pipelines/musicbrainz/src/musicbrainz/silver/songs.py`) — a `(title, artist, youtube_video_id, genre_name)` dataset of MusicBrainz-derived songs, built for a downstream consumer (`the-music-tree-api`'s genre-tree endpoint). Joins `1_recording_link`, `2_recording_genre`, and Bronze `genre`/`recording`/`artist_credit_name`/`artist`:
...
- **Video id extraction** handles `youtu.be/<id>`, `youtube.com/watch?v=<id>`, `youtube.com/embed/<id>`, and `youtube.com/v/<id>`, capturing exactly 11 characters — a real YouTube video id's fixed length. This both drops URLs with fewer than 11 valid characters (a truncated/malformed id) and stops a stray trailing character (seen in real MusicBrainz data) from being swallowed into the captured id. A bare playlist/channel URL (e.g. `youtube.com/playlist?list=...`) carries no video id in any of those positions and is dropped, not defaulted to a null/garbage id — there's no video to point at.
- **Primary genre = highest `weight` per recording** (ties broken by lowest `genre_id`, for determinism) — `2_recording_genre` is deliberately many-to-many and leaves "pick one genre" as a consumer decision (see [2. Silver](#2-silver) above); this step is that consumer, since the output needs exactly one genre per song.
- **No genre-tree name filtering here**: this step emits the recording's matched `genre.name` as-is, unfiltered and untranslated against wikidata's canonical genre names — reconciling raw tag names against the canonical tree now happens downstream in `pipelines/gold` (see `pipelines/gold/DESIGN.md`), keeping this Silver layer itself genre-tree-agnostic.
...
- Run via `uv run --package musicbrainz python -m musicbrainz.silver`, writing `SILVER_OUTPUT_DIR/3_songs.parquet`.
- **On-demand JSON export**: `scripts/export_songs_json.py` reads `3_songs.parquet` and writes a flat JSON array (`[{"title": ..., "artist": ..., "youtube_video_id": ..., "genre_name": ...}, ...]`) for a developer to manually copy/commit into the downstream API repo. Run with `uv run --package musicbrainz python scripts/export_songs_json.py <output.json>` whenever a fresh copy is needed — not a scheduled job, no `infrastructure` involvement.
