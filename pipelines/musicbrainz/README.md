...
| Silver | `recording_link` (recording ↔ link correspondence, typed via `link_type`, e.g. YouTube/streaming/official homepage, built); `recording_genre` (cleaned recording ↔ genre associations, built); `recording_genre_youtube` (recording ↔ genre ↔ YouTube-URL join, built); `genre_hierarchy` (parent/child from Wikidata), `recording_genre_path` (final recording → genre-path join) — not yet built |
