...
`tests/` mirrors the `src/musicbrainz/` module layout. Test files are `test_*.py`, test functions are `test_*` — standard `pytest` discovery, no custom configuration needed beyond `testpaths` (already set in `pyproject.toml`).
