...
**`1_canonical_genre_tree.json`** — `{"tree": [<node>, ...]}`, one entry per root of `7_canonical_hierarchy`. A node is `{"id": str, "name": str, "children": [<node>, ...], "side": "pop" (optional, direct children of a root only)}`, recursive (validated against `src/gold/schemas/genre_tree.schema.json`), building on `the-music-tree-genre-kit`'s `CriteriaTreeImportSerializer`/`TreeField` import shape. `id` is the node's Wikidata QID (`item_id`), a stable identifier across label changes — not yet consumed on the `grow-the-music-tree-api` side. `side` is only ever present with value `"pop"`, per `manual_canonical_genre_pop_side.csv` below — see [DESIGN.md](DESIGN.md).
