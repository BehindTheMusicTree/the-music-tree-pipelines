...
- `pipelines/musicbrainz` — Bronze ingestion from a Postgres MusicBrainz mirror (6 tables: `recording`, `tag`, `recording_tag`, `genre`, `url`, `l_recording_url`).
...
- `musicbrainz`: connects to Postgres via `psycopg`, reads each of the 6 raw tables with Polars (`pl.read_database`), writes one Parquet file per table to `BRONZE_OUTPUT_DIR`. See `pipelines/musicbrainz/src/musicbrainz/{ingest.py,db.py}`.
