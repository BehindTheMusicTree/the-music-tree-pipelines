...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (five steps, `1_item_links` → `5_hierarchy` — see below), and `musicbrainz` (one step so far, `1_recording_youtube_url` — see below). Gold layer is planned but not yet built.
...
**Silver layer, `musicbrainz` (`pipelines/musicbrainz/src/musicbrainz/silver/`):** one step so far. `1_recording_youtube_url` joins Bronze `l_recording_url` to `url` (filtered to `youtube.com`/`youtu.be`, since `link_type` isn't ingested at Bronze — see `pipelines/musicbrainz/SCHEMA.md#bronze`) to produce a recording ↔ YouTube-URL correspondence, many-to-many (a recording can have zero, one, or several YouTube links). See `pipelines/musicbrainz/SCHEMA.md#silver` for the full derivation. Planned next steps (`recording_genre`, `genre_hierarchy`, `recording_genre_path`) are not yet built.
