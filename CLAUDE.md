...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (six steps, `1_item_links` → `6_canonical_roots` — see below), and `musicbrainz` (three steps so far, `1_recording_link` → `3_song_example` — see below). Gold layer is planned but not yet built.
...
- **Run wikidata Silver:** `uv run --package wikidata python -m wikidata.silver` (reads `BRONZE_OUTPUT_DIR/wikidata_genre_tree.parquet`, runs all six steps in sequence, writes `SILVER_OUTPUT_DIR/1_item_links.parquet`, `2_regional_overview_classification.parquet`, `3_regional_classification.parquet`, `4_genre_parents.parquet`, `5_hierarchy.parquet`, `5_regional_hierarchy.parquet`, and `6_canonical_roots.parquet`)
...
**Silver layer, `wikidata` (`pipelines/wikidata/src/wikidata/silver/`):** six sequential steps, each reading the previous step's output. `1_item_links` derives a browsable Wikidata page URL (`item_url`/`parent_url`) from each row's `item_id`/`parent_id`, and adds `has_item_label`/`has_parent_label` flags, `False` where the label service fell back to printing the QID (no label in any queried language). `2_regional_overview_classification` flags (not drops) rows whose `item_label` is a Wikidata "music of \<place\>" regional-overview article (e.g. "music of Kenya") rather than an actual genre — it also reads a git-tracked, hand-curated `manual_regional_overview_additions.csv` as a backstop for real overview items entirely absent from Bronze (not even as a `parent_label`). `3_regional_classification` cascades that seed set down through parent edges to flag every nationally/ethnically-specific genre (e.g. "fado", "morna") as regional, as opposed to canonical genres like "rock music" — it also reads a git-tracked, hand-curated `manual_regional_overrides.csv` (`regional_reason = "manual_override"`) as a backstop for the rare genre none of the automated seed/`indigenous_to`/`country_of_origin` sources catch. `4_genre_parents` flags whether each row's parent is itself a real genre. `5_hierarchy` is the first step that prunes rather than flags: it collapses the edge list to one row per genre item and splits it into two outputs — `5_hierarchy.parquet` (canonical, excludes regional items) and `5_regional_hierarchy.parquet` (regional items only). `6_canonical_roots` extracts `5_hierarchy`'s root items (no parent) into their own `6_canonical_roots.parquet`, for manual exploration of the "too many roots" open question below. See `pipelines/wikidata/SCHEMA.md#silver` for the full column definitions, rules, and profiling detail — several rules (e.g. the regional cascade's seed set, the multi-parent collapse heuristic) are explicitly marked provisional/exploration-phase there. `5_hierarchy` (canonical) currently surfaces a high number of root items; whether that reflects the source data or an upstream pruning artifact is an open exploration — see `pipelines/wikidata/notebooks/explore_genre_tree.ipynb`.
