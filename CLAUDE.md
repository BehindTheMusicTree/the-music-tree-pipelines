...
**Silver layer, `musicbrainz` (`pipelines/musicbrainz/src/musicbrainz/silver/`):** two steps so far. `1_recording_link` joins Bronze `l_recording_url` → `link` → `link_type` → `url` (see `pipelines/musicbrainz/SCHEMA.md#bronze`) to produce a recording ↔ link correspondence typed by `link_type.name` (free streaming, streaming, license, etc.), many-to-many (a recording can have zero, one, or several links of any type/platform). `2_recording_genre` joins Bronze `recording_tag` to `tag` to `genre` (case-insensitive name match, `count > 0`) to produce a recording ↔ genre correspondence, many-to-many. See `pipelines/musicbrainz/SCHEMA.md#silver` for the full derivations. Both steps stay tidy/long (one row per link, one row per recording-genre match) — collapsing to one row per recording with list-valued link/genre columns is a Gold-layer concern (not yet built), not Silver's. Planned next step (`recording_genre_path`) is not yet built.
