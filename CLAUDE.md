...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (five steps, `1_item_links` → `5_hierarchy` — see below), and `musicbrainz` (three steps so far, `1_recording_link` → `3_song_example` — see below). Gold layer is planned but not yet built.
