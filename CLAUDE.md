...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (five steps, `1_item_links` → `5_hierarchy` — see below), and `musicbrainz` (two steps so far, `1_recording_youtube_url` and `2_recording_genre` — see below). Gold layer is planned but not yet built.

- `pipelines/common` — shared utilities (currently just per-pipeline `.env` loading).
- `pipelines/musicbrainz` — Bronze ingestion from a Postgres MusicBrainz mirror (8 tables: `recording`, `tag`, `recording_tag`, `genre`, `url`, `l_recording_url`, `link`, `link_type`), plus a Silver pipeline (`musicbrainz.silver`) that derives a recording ↔ YouTube-URL correspondence.
...
- **Run musicbrainz Silver:** `uv run --package musicbrainz python -m musicbrainz.silver` (reads `BRONZE_OUTPUT_DIR/url.parquet` and `BRONZE_OUTPUT_DIR/l_recording_url.parquet`, writes `SILVER_OUTPUT_DIR/1_recording_youtube_url.parquet`)
...
**Silver layer, `musicbrainz` (`pipelines/musicbrainz/src/musicbrainz/silver/`):** two steps so far. `1_recording_youtube_url` joins Bronze `l_recording_url` to `url` (filtered to `youtube.com`/`youtu.be` by substring match — `link_type` is ingested at Bronze but not yet used here, see `pipelines/musicbrainz/SCHEMA.md#bronze`) to produce a recording ↔ YouTube-URL correspondence, many-to-many (a recording can have zero, one, or several YouTube links). `2_recording_genre` joins Bronze `recording_tag` to `tag` to `genre` (case-insensitive name match, `count > 0`) to produce a recording ↔ genre correspondence, many-to-many. See `pipelines/musicbrainz/SCHEMA.md#silver` for the full derivations. Goal: one row per recording carrying its links (YouTube, Spotify, etc.) and its MusicBrainz genres/tags. Planned next steps (`recording_genre_path`, more link platforms beyond YouTube via `link_type`) are not yet built.
