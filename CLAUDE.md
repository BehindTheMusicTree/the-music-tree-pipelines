...
A `uv` workspace monorepo of data pipelines feeding the BTMT (Behind The Music Tree) ecosystem. Each pipeline is an independent workspace member under `pipelines/`, sharing only the `common` package. All pipelines implement a **Bronze layer** (raw extraction to Parquet); a **Silver layer** (cleaned, joined, hierarchy-built data) is underway for both pipelines: `wikidata` (five steps, `1_item_links` → `5_hierarchy` — see below), and `musicbrainz` (three steps so far, `1_recording_link`, `2_recording_genre`, and `3_recording_genre_youtube` — see below). Gold layer is planned but not yet built.

- `pipelines/common` — shared utilities (currently just per-pipeline `.env` loading).
- `pipelines/musicbrainz` — Bronze ingestion from a Postgres MusicBrainz mirror (8 tables: `recording`, `tag`, `recording_tag`, `genre`, `url`, `l_recording_url`, `link`, `link_type`), plus a Silver pipeline (`musicbrainz.silver`) that derives a recording ↔ link correspondence (typed by platform via `link_type`) and a recording ↔ genre correspondence.
...
**Silver layer, `musicbrainz` (`pipelines/musicbrainz/src/musicbrainz/silver/`):** three steps so far. `1_recording_link` joins Bronze `l_recording_url` → `link` → `link_type` → `url` (see `pipelines/musicbrainz/SCHEMA.md#bronze`) to produce a recording ↔ link correspondence typed by `link_type.name` (free streaming, streaming, license, etc.), many-to-many (a recording can have zero, one, or several links of any type/platform). `2_recording_genre` joins Bronze `recording_tag` to `tag` to `genre` (case-insensitive name match, `count > 0`) to produce a recording ↔ genre correspondence, many-to-many. `3_recording_genre_youtube` composes those two Silver outputs, inner-joined on `recording_id` and filtered to `1_recording_link`'s YouTube-domain rows (substring match on `url`, since `link_type` categorizes relationship kind, not platform), to produce a recording ↔ genre ↔ YouTube-URL correspondence, many-to-many. See `pipelines/musicbrainz/SCHEMA.md#silver` for the full derivations. Goal: one row per recording carrying its links (YouTube, Spotify, etc.) and its MusicBrainz genres/tags. Planned next step (`recording_genre_path`) is not yet built.
