...
- `wikidata` Silver layer, `3_hierarchy`: the first Silver step that drops rows rather than flagging. Prunes `2_genre_parents.parquet` to genre-only edges, then collapses each remaining multi-parent item down to a single row by keeping the parent with the lowest numeric QID — **a provisional, tâtonnement heuristic, not a settled design decision**: ~43% of genre items (2,727 of ~6,337) have more than one surviving genre parent, and only 1 item in the whole genre extension has a Wikidata "preferred rank" `P279` statement, so no reliable disambiguation signal currently exists; expect this rule to be revisited once a real product/curation decision is made. `item_id` is unique in this output, unlike every prior Bronze/Silver artifact. `wikidata.silver.profile` now also reports the vanished-items count (genre items whose every parent edge was non-genre). See `SCHEMA.md#3_hierarchy`.
- `wikidata` Silver layer, `3_regional_classification`: adds `is_regional`/`regional_reason` columns, cascading a "regional" flag (nationally/ethnically specific genres, e.g. morna, fado, variété française) down from `1_classification`'s `regional_overview` seeds (e.g. "music of Cape Verde") to every descendant genre item, via an **any-parent** rule — an item is regional if *any* one of its parent edges points to a seed or an already-regional item, not only when *every* path dead-ends there. This was confirmed necessary on live data: "Portuguese folk music" has one parent edge into the "music of Portugal" seed and one into "European folk music," and the latter is itself a regional genre, so an all-paths rule would have missed it. Flag-only, no rows dropped. **Exploration-phase finding, expected to evolve**: on a real run, ~51% of all genre items (3,078 of 6,039) end up flagged `is_regional` — far more than initially expected — because several of the `regional_overview` seeds are continent-level ("music of Asia", "music of Europe", ...) with very large fan-out (avg. ~4.7 direct children across 299 seeds). Kept as-is for now rather than narrowing the seed set, since the design is still exploratory; see `SCHEMA.md#3_regional_classification`'s "Exploration phase" callout. Also flags, as a follow-up investigation (not a code gap): "variété française" doesn't appear anywhere in the current Bronze extraction at all, so it can't be classified either way.
- `wikidata` Silver layer, `4_hierarchy` (renumbered from `3_hierarchy`): now produces **two** outputs instead of one — `4_hierarchy.parquet` (canonical, excluding every `is_regional = true` item, same pruning behavior as before) and `4_regional_hierarchy.parquet` (regional, containing only `is_regional = true` items). The two outputs diverge on how they handle items with no genre/regional parent: canonical still drops such items entirely (silent vanish, unchanged behavior — e.g. "opera"), while regional instead promotes them to a root of the regional graph (e.g. morna, whose only parent is its own non-genre `regional_overview` seed) so regional items never disappear silently. Confirmed on live data that all 984 items promoted to regional roots had a real parent edge in the source data (none were genuine pre-existing Wikidata roots). `wikidata.silver.profile`'s `profile_hierarchy` now reports both outputs' row counts plus the count of genre items surviving in *neither* output. See `SCHEMA.md#4_hierarchy`.
