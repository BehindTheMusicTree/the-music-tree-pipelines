...
- `wikidata` Silver layer, `1_genre_classification`: flags each Bronze genre-tree row `is_genre`/`classification_reason`, tagging (not dropping) Wikidata's "music of \<place\>" regional-overview items (~300 of ~6,300) that were being misclassified as music genres by the `P31` source query — these become the seed set for `2_regional_classification` rather than being excluded outright.
- `wikidata.silver.profile`: read-only script printing row/item counts and the `classification_reason` breakdown for `1_genre_classification.parquet` — see `SCHEMA.md#silver`'s "Data profile" section.
