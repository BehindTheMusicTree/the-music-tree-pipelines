...
- `wikidata` Bronze: widen the `SERVICE wikibase:label` fallback chain from `"en"` to `"en,mul"` in all three SPARQL queries, so items with no English label but a language-agnostic (`mul`) one get a real label instead of the QID-string fallback. `wikidata` Silver: `1_item_links` adds `has_item_label`/`has_parent_label` boolean flags (`item_label == item_id` / `parent_label == parent_id`) so later steps that pattern-match on label text don't silently match against a QID for items still unlabeled in any language. See `pipelines/wikidata/SCHEMA.md`.
